s# Gratitude Journal — Production-ready Scaffold
...
<form method="post" action="{{ url_for('entries.add') }}">
  <textarea name="content" rows="4" placeholder="Today I'm grateful for..." required></textarea>
...
<ul class="entries">
  {% for e in entries %}
    <li>
      <time>{{ e.created_at.strftime('%Y-%m-%d') }}</time>
      <p>{{ e.content }}</p>
    </li>
  {% else %}
    <li>No entries yet — add your first!</li>
  {% endfor %}
</ul>
{% endblock %}
```
...
nav { display:flex; justify-content:flex-end; gap:1rem; margin-bottom: 1rem; }
textarea { width:100%; }
.entries { list-style:none; padding:0; }
.entries li { border-bottom: 1px solid #eee; padding: .75rem 0; }
.flashes { color: #b00; }
```
...
* **Migrations**: after first run, inside web container run `flask db init` (if not present), `flask db migrate`, `flask db upgrade`.
* **Email / password reset**: integrate Flask-Mail + token endpoints.
* **CSRF**: consider adding Flask-WTF for forms with CSRF tokens (important for production).
* **Rate limiting**: add Flask-Limiter for login throttling.

---

If you want, I can:

* generate `alembic` migration env & initial migration,
* add CI (GitHub Actions) to run tests / build images,
* swap Nginx for Traefik and show a Let’s Encrypt automatic TLS config.
